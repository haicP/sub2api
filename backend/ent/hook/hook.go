...
// The RequestDetailFunc type is an adapter to allow the use of ordinary
// function as RequestDetail mutator.
type RequestDetailFunc func(context.Context, *ent.RequestDetailMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RequestDetailFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RequestDetailMutation); ok {
		return f(ctx, mv)
	}
